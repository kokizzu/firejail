...
for app in "${apps[@]}"; do
	if command -v "$app"
	then
		echo "TESTING: private-lib $app (test/private-lib/$app.exp)"

		./"$app".exp ||
		echo "TESTING ERROR: test/private-lib/$app.exp: $?"
	else
		echo "TESTING SKIP: $app not found (test/private-lib/$app.exp)"
	fi
done
